...
class JoueurController extends Controller
{
    ...
    public function newAction(Request $request)
    {
        ...
        $api =file_get_contents('https://randomuser.me/api/?nat=fr');
        $jsonapi =  json_decode($api, true);
        $nom =$jsonapi['results'][0]['name']['first'];
//        $form->getData()->setNom($nom);
        if ($form->isSubmitted() && $form->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $form->getData()->setExperiance(rand(20, 80));
            $form->getData()->setAge(rand(18, 30));
            $form->getData()->setFatigue(rand(0, 20));
            $em->persist($joueur);
            $em->flush();

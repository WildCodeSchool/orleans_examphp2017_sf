...
class EquipeController extends Controller
{
     ...
     * @Method({"GET", "POST"})
     */
    public function searchAction(Request $request, Search $search)
    {
        $searchForm = $this->searchForm($search);
        $searchForm = $this->createForm('QuidditchBundle\Form\SearchType', $search);
        $searchForm->handleRequest($request);

        if ($searchForm->isSubmitted() && $searchForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('search', array('id' => $search->findAll()));
        }
